...
async function fetchLogs() {
  loading.value = true
  try {
    logs.value = await api.getWhisperClusterLogs(instanceName.value, Number(lines.value))
  } catch {
...
function highlightLine(line: string): string {
  if (line.includes('failed') || line.includes('error') || line.includes('Error') || line.includes('ERROR') || line.includes('[錯誤]')) {
    ...
    <DialogContent max-width="max-w-3xl" @close="close">
      ...
      <div ref="logsContainer" class="rounded-md border bg-zinc-950 overflow-auto" style="height: 400px">
        <pre class="p-3 text-xs font-mono text-zinc-200 leading-relaxed whitespace-pre-wrap break-all">
          ...
          <template v-else>
            <span v-for="(line, idx) in logs" :key="idx" :class="highlightLine(line)">{{ line }}<br/></span>
          </template>
        </pre>
      </div>
    </DialogContent>
